...
namespace ACGPlugin {
    ...
    inline std::optional<omp_mode> parse_omp_mode() {
    ...
    struct omp {
        omp(metacg::Callgraph& mcg, llvm::CallGraph const& lcg) : mode{parse_omp_mode()}, mcg{mcg}, lcg{lcg} {}

        void add(llvm::CallBase const& call) const {
        ...
        [[nodiscard]] auto has_enabled_mode(omp_mode const m) const { return mode.has_value() && *mode == m; }

        [[nodiscard]] auto has_not_enabled_mode(omp_mode const m) const { return mode.has_value() && *mode != m; }

        [[nodiscard]] auto enabled() const { return mode.has_value(); }

    private:
        void add_merged(metacg::CgNode const* parent, llvm::CallBase const& call) const {
